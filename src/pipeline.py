    ...
    for stock in stocks.to_dict("records"):
        ...
        try:
            ...
            for horizon in HORIZONS:
                try:
                    result=fit_forecast(features,FEATURE_COLUMNS,f"target_{horizon}");bt=walk_forward_backtest(features,FEATURE_COLUMNS,f"target_{horizon}",80,12,42,result.model_name);summary=summarize_backtest(bt)
                    ...
                    row[f"{horizon}_status"]="ok";row[f"{horizon}_raw_predicted_price"]=result.predicted_price;row[f"{horizon}_predicted_price"]=predicted_price;row[f"{horizon}_expected_return"]=calibrated_return
                    row[f"{horizon}_calibration_adjustment"]=adjustment;row[f"{horizon}_calibration_samples"]=calibration_samples;row[f"{horizon}_error_band"]=band
                    row[f"{horizon}_lower_price"]=max(0.0,latest_price*(1+lower_return));row[f"{horizon}_upper_price"]=max(0.0,latest_price*(1+upper_return))
                    ...
                    backtest_rows.append({"run_at_utc":run_time,"symbol":symbol,"name":stock["name"],"horizon":horizon,**summary,"selected_model":result.model_name,"calibration_adjustment":adjustment,"calibration_samples":calibration_samples,"error_band":band})
                except Exception as exc:
                    row[f"{horizon}_status"]=f"unavailable: {exc}"
                    for suffix in ("raw_predicted_price","predicted_price","expected_return","calibration_adjustment","calibration_samples","error_band","lower_price","upper_price","validation_mae","validation_samples","training_samples","confidence","model","candidate_mae_hist","candidate_mae_extra_trees","candidate_mae_zero"): row[f"{horizon}_{suffix}"]=pd.NA
